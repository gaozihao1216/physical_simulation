"""Deterministic convex mesh generation for analytic geometry."""
...
class ConvexMeshSpec:
    """Inline convex mesh data using local vertices and triangular faces."""
...
def geometry_to_convex_mesh(geometry: GeometrySpec) -> ConvexMeshSpec:
    ...
    if isinstance(geometry, WedgeGeometry):
        return wedge_to_convex_mesh(geometry)
    if isinstance(geometry, ConeGeometry):
        return cone_to_convex_mesh(geometry, segments=32)
    if isinstance(geometry, FrustumGeometry):
        return frustum_to_convex_mesh(geometry, segments=32)
    if isinstance(geometry, RegularPrismGeometry):
        return regular_prism_to_convex_mesh(geometry)
    raise InvalidGeometryError(f"unsupported geometry for convex mesh fallback; geometry={geometry!r}")


def wedge_to_convex_mesh(geometry: WedgeGeometry) -> ConvexMeshSpec:
    """Return a triangular prism mesh for a wedge/ramp geometry."""
    x, y, z = geometry.size
    hx = x / 2.0
    hy = y / 2.0
    hz = z / 2.0
...
def cone_to_convex_mesh(geometry: ConeGeometry, *, segments: int) -> ConvexMeshSpec:
    """Return a polygonal cone mesh with a deterministic segment count."""
    if segments < 3:
...
def frustum_to_convex_mesh(geometry: FrustumGeometry, *, segments: int) -> ConvexMeshSpec:
    """Return a polygonal frustum mesh with a deterministic segment count."""
    if segments < 3:
...
def regular_prism_to_convex_mesh(geometry: RegularPrismGeometry) -> ConvexMeshSpec:
    """Return a regular polygon prism mesh."""
